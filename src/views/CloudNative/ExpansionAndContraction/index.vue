    ...
    >
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="240">
        <template slot-scope="scope">
          ...
          <!--          <el-button v-if="checkPermission(['admin'])" type="primary" size="mini" v-on:click="deleteBms(row)">删除-->
          <!--          </el-button>-->
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    viewBms(row) {
      ...
      this.axios.get('/autoscaling/' + userId + '/' + row.clusterId).then(res => {
        console.log(res)
        if (res.status === 200) {
          this.$router.push({
            path: '/nativeCloud/ExpansionAndContraction/detail',
            query: {
              clusterId: row.clusterId
            }
          })
        } else {
          this.$message.warning('尚不能进行自动扩缩容！')
        }
